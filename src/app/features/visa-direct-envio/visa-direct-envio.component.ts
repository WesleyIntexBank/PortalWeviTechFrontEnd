...
import { VisaDirectKafkaService, VisaDirectPayload, VisaDirectKafkaResultado } from '../../core/services/visa-direct-kafka.service';

@Component({
  selector: 'app-visa-direct-envio',
  standalone: true,
  imports: [
    CommonModule,
    ReactiveFormsModule,
  ...
  template: `
    <div class="page-wrapper">
      ...
      <div class="page-header">
        ...
        <div>
          <h1>Envio Visa Direct</h1>
          <p class="subtitle">Monte a lista de registros e envie para processamento via Kafka → WorkerVisaDirect</p>
        </div>
      </div>
              ...
              <mat-accordion class="form-accordion" multi>
                ...
                <mat-expansion-panel>
                  ...
                  <div class="form-row">
                    ...
                    <mat-form-field appearance="outline" class="field-md">
                      ...
                      <input matInput formControlName="ROUTING_NUMBER">
                    </mat-form-field>
                    <mat-form-field appearance="outline" class="field-lg">
                ...
                <mat-expansion-panel>
                  ...
                  <div class="form-row">
                    ...
                    <mat-form-field appearance="outline" class="field-md">
                      <mat-label>Cidade Pagador (PAGADORCD)</mat-label>
                    ...
                    <mat-form-field appearance="outline" class="field-md">
                      <mat-label>Conta Pagador (PAGADORC)</mat-label>
                    ...
                    <tbody>
                      @for (item of lista(); track $index) {
                        <tr [class.row-success]="isSuccess($index)" [class.row-error]="isError($index)">
                          ...
                          <td class="td-code">{{ item.ACCOUNT_NUMBER || '—' }}</td>
                          <td class="td-code">{{ item.ROUTING_NUMBER || '—' }}</td>
                          <td class="td-actions">
  ...
  styles: [`
    ...
    .header-icon-wrap { display: flex; align-items: center; justify-content: center; width: 48px; height: 48px; border-radius: 12px; background: rgba(25, 118, 210, 0.12); flex-shrink: 0; }
    .header-icon { font-size: 26px; width: 26px; height: 26px; color: #1976d2; }
    ...
    ::ng-deep input[type="date"]::-webkit-calendar-picker-indicator { filter: var(--calendar-icon-filter, invert(0.6)); cursor: pointer; }
    ...
    ::ng-deep .mat-expansion-indicator::after { color: var(--text-sec) !important; }
    ::ng-deep .form-card .mat-mdc-card-title { color: var(--text) !important; }
    ::ng-deep .form-card .mat-mdc-card-subtitle { color: var(--text-sec) !important; }
    ::ng-deep .list-card .mat-mdc-card-title { color: var(--text) !important; }
    ::ng-deep .list-card .mat-mdc-card-subtitle { color: var(--text-sec) !important; }
    ...
    .card-avatar-icon { color: #1976d2; font-size: 26px; width: 26px; height: 26px; }
    ...
    .form-accordion mat-expansion-panel-header { border-radius: 10px !important; }
    .badge { display: inline-flex; align-items: center; justify-content: center; background: #1976d2; color: #fff; border-radius: 10px; min-width: 20px; height: 20px; padding: 0 6px; font-size: 11px; font-weight: 800; margin-left: 10px; vertical-align: middle; }
    ...
    .chip-moeda { display: inline-block; background: rgba(25, 118, 210, 0.15); color: #1976d2; border: 1px solid rgba(25, 118, 210, 0.3); border-radius: 6px; padding: 1px 7px; font-size: 10px; font-weight: 700; letter-spacing: 0.5px; }
    .row-success td { background: rgba(25, 118, 210, 0.06) !important; }
    .row-error   td { background: rgba(239, 83, 80, 0.06) !important; }
    .status-icon { vertical-align: middle; font-size: 20px; }
    .icon-ok  { color: #1976d2; }
    ...
    .result-item { display: flex; align-items: flex-start; gap: 10px; padding: 10px 12px; border-radius: 8px; font-size: 13px; border: 1px solid transparent; }
    .result-ok { background: rgba(25, 118, 210, 0.08); border-color: rgba(25, 118, 210, 0.2); }
    .result-err { background: rgba(239, 83, 80, 0.08); border-color: rgba(239, 83, 80, 0.2); }
    .result-ok  mat-icon { color: #1976d2; font-size: 20px; }
...
export class VisaDirectEnvioComponent {
  private service = inject(VisaDirectKafkaService);
  ...
  form = new FormGroup({
    ...
    ROUTING_NUMBER: new FormControl('', { nonNullable: true }),
  ...
  lista      = signal<VisaDirectPayload[]>([]);
  enviando   = signal(false);
  resultados = signal<VisaDirectKafkaResultado[]>([]);

  resultadosMap = computed<Record<number, VisaDirectKafkaResultado>>(() => {
    const map: Record<number, VisaDirectKafkaResultado> = {};
  ...
  adicionar(): void {
    ...
    const payload: VisaDirectPayload = {
      ...v,
